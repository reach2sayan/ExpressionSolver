...
include/md/third_party/mdspan.hpp is the kokkos/mdspan single-header build run
through our .clang-format, so a plain diff against upstream is unreadable:
clang-format re-splits long string literals, rewraps every comment and sorts
includes.  This normalises both sides -- comments and whitespace out, line
continuations joined, preprocessor directives kept whole, adjacent string
literals merged back together -- and diffs what is left, which is the code.
...
# The commit include/md/third_party/mdspan.hpp was taken from.  Bump on re-vendor.
PINNED = "c3fc07b607db2a43cd65aabc12ffda9328833d95"
VENDORED = Path(__file__).resolve().parent.parent / "include/md/third_party/mdspan.hpp"
...
def normalize(src: str) -> list[str]:
    ...
    for line in text.split("\n"):
        ...
        if not s:
            continue
        if s.startswith("#"):
            toks.append("#DIRECTIVE " + " ".join(TOKEN.findall(s)))
        else:
            toks.extend(TOKEN.findall(s))
    # "abc " "def" and "abc def" are the same string to the compiler, and
    # clang-format moves the split around freely.
    merged: list[str] = []
    for t in toks:
        if t.startswith('"') and t.endswith('"') and merged[-1:] and \
                merged[-1].startswith('"') and merged[-1].endswith('"'):
