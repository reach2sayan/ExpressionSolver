...
TEST(RecordTest, KeepsTheValueTypeOfEachEntry) {
...
TEST(RecordTest, SubscriptReadsTheSameSlotAsGet) {
  constexpr auto m = record(named<"n">(3), named<"x">(1.5));
  EXPECT_EQ(m["n"_s], m.get<"n">());
  EXPECT_DOUBLE_EQ(m["x"_s], m.get<"x">());
}
TEST(RecordTest, OwnershipFollowsTheRecordNotTheKey) {
  auto m = record(named<"x">(1.5));
  static_assert(std::is_same_v<decltype(m.get<"x">()), double &>);
  static_assert(
      std::is_same_v<decltype(std::as_const(m).get<"x">()), const double &>);
  static_assert(
      std::is_same_v<decltype(record(named<"x">(1.5)).get<"x">()), double>);
}
TEST(RecordTest, ContainsAnswersForAbsentAndSimilarKeys) {
  constexpr auto m = record(named<"x">(1.5));
  static_assert(m.contains<"x">());
  static_assert(!m.contains<"y">());
  static_assert(!m.contains<"xy">());
}
TEST(RecordTest, EverySpellingBuildsTheSameType) {
  constexpr Record by_keyword{named<"n">(3), named<"x">(1.5)};
  constexpr Record by_label{Entry{"n"_s, 3}, Entry{"x"_s, 1.5}};
  constexpr Record by_variable{Entry{kRecordN, 3}, Entry{kRecordX, 1.5}};
  constexpr Record by_tagged_named{named(kRecordN, 3), named(kRecordX, 1.5)};
  constexpr Record<Entry<"n", int>, Entry<"x", double>> by_type{{3}, {1.5}};
...
TEST(RecordTest, KeysAreInEntryOrderNotSorted) {
  constexpr auto m = record(named<"z">(1), named<"a">(2), named<"m">(3));
  static_assert(m.keys()[0] == std::string_view{"z"});
  static_assert(m.keys()[1] == std::string_view{"a"});
  static_assert(m.keys()[2] == std::string_view{"m"});
}
TEST(RecordTest, SetWritesTheNamedSlotInPlace) {
  auto m = record(named<"a">(1.0), named<"b">(2.0));
  m.set<"a">(7.0);
  m["b"_s] = 9.0;
  EXPECT_DOUBLE_EQ(m.get<"a">(), 7.0);
  EXPECT_DOUBLE_EQ(m.get<"b">(), 9.0);
}
TEST(RecordTest, InsertAndEraseReturnNewRecords) {
  constexpr auto m = record(named<"n">(3), named<"x">(1.5));
...
TEST(RecordTest, EraseThenInsertRetypesASlot) {
  constexpr auto m = record(named<"n">(3), named<"x">(1.5));
...
TEST(RecordTest, ForEachVisitsEveryEntryInOrder) {
  constexpr auto m = record(named<"n">(3), named<"x">(1.5));
  std::string labels;
  double sum = 0.0;
  ...
  auto mutable_map = record(named<"a">(1.0), named<"b">(2.0));
  mutable_map.for_each([](auto, auto &v) { v *= 10.0; });
  EXPECT_DOUBLE_EQ(mutable_map.get<"a">(), 10.0);
  EXPECT_DOUBLE_EQ(mutable_map.get<"b">(), 20.0);
}
TEST(RecordTest, HoldsValuesNoExpressionWouldAccept) {
  constexpr auto m = record(named<"name">(std::string_view{"ddx"}),
...
TEST(RecordTest, EqualityIsKeysInOrderAndValues) {
  constexpr auto m = record(named<"n">(3), named<"x">(1.5));
  static_assert(m == record(named<"n">(3), named<"x">(1.5)));
  static_assert(!(m == record(named<"n">(4), named<"x">(1.5))));
  static_assert(!std::is_same_v<decltype(m), decltype(record(named<"x">(1.5),
                                                             named<"n">(3)))>);
}
TEST(RecordTest, EmptyRecordIsUsable) {
  constexpr Record<> m{};
...
// Relaxing Entry to any object type must not disturb its numeric use.
TEST(RecordTest, EntryStillDrivesTheExpressionSide) {
