...
from ._ddx import (
    ...
    Expression,
    VecLib,
    abs,  # noqa: A004  -- the opcode is spelled `abs`, as it is in C++
    acos,
    acosh,
    add,
    asin,
...
try:
    __version__ = version("ddx")
except PackageNotFoundError:  # an in-tree build that was never installed
    __version__ = "0+unknown"


# The two places pydantic sits between a caller and the struct.  Attached here
# rather than bound in C++ so that validation is not something a caller can go
# around: _options is the raw one, and nothing else uses it.
...
# Any is the honest annotation: the keywords are Options' fields, and Options is
# where they are checked -- a TypedDict here would restate that list to say less.
...
# The declarations above say these exist; these are what make them.
